...
| 인증 | Firebase Authentication | 이메일/비밀번호 인증 (회원가입/로그인) |
...
| 통계 대시보드 | 카테고리 분포, 월별 대여 추이, 상위 대여자, 대여 기간 분석 (SVG 차트) |
| 토스트 알림 | CRUD 작업 완료 시 성공/실패/정보 알림 표시 |
| 스켈레톤 로딩 | 데이터 로딩 중 콘텐츠 형태의 플레이스홀더 표시 |
...
| `/stats` | 통계 대시보드 | 카테고리 분포, 월별 대여, 상위 대여자, 대여 기간 분석 |
...
│   │   │   ├── Card.tsx            카드 컨테이너 (glass-morphism)
│   │   │   ├── EmptyState.tsx      빈 상태 안내 (float 애니메이션)
│   │   │   ├── Input.tsx           텍스트 입력
│   │   │   ├── Loading.tsx         로딩 스피너
│   │   │   ├── Modal.tsx           모달(팝업) 창 (bounce-in 애니메이션)
│   │   │   ├── Select.tsx          드롭다운 선택
│   │   │   ├── Skeleton.tsx        스켈레톤 로딩 (카드/통계/친구 타입별)
│   │   │   ├── Toast.tsx           토스트 알림 (success/error/info)
...
- **색상 팔레트**: Indigo-Violet 그라디언트 (Primary #6366f1 기반 10단계)
- **Glass-morphism**: `backdrop-blur` + 반투명 배경으로 깊이감 표현
- **그라디언트 버튼**: Primary/Danger 버튼에 그라디언트 + 컬러 그림자 적용
- **둥근 모서리**: `rounded-2xl` ~ `rounded-3xl` 카드/모달로 부드러운 느낌

#### 레이아웃

- **모바일 우선**: 하단 탭 내비게이션 (모바일) / 왼쪽 사이드바 (데스크탑)
- **반응형 헤더**: 데스크탑 사용자 카드 / 모바일 아이콘 버튼 분기
- **사이드바**: 활성 메뉴 그라디언트 + shimmer 효과, 하단 사용자 카드
- **모바일 하단 탭**: 활성 탭 상단 인디케이터 바, scale 애니메이션

#### 애니메이션 & 마이크로 인터랙션

| 애니메이션 | 적용 위치 | 설명 |
|-----------|----------|------|
| `fade-in` | 페이지 전체 | 페이지 진입 시 부드러운 등장 |
| `bounce-in` | 모달, 로그인, 웰컴 카드 | 탄성 있는 스프링 등장 효과 |
| `slide-in-right` | 토스트 알림 | 우측에서 슬라이드 인 |
| `float` | 빈 상태 아이콘, 배경 장식 | 부드러운 부유 효과 |
| `shimmer` | 스켈레톤, 사이드바 활성 메뉴 | 좌→우 빛 반사 효과 |
| 스태거 딜레이 | 목록 카드, 통계 그리드 | 순차적 등장 (`40~80ms` 간격) |
| `active:scale` | 버튼, 카드, 네비게이션 | 클릭 시 눌림 피드백 |
| `hover:-translate-y` | 카드 | 호버 시 살짝 떠오르는 효과 |

#### 로딩 & 피드백

- **스켈레톤 로딩**: 모든 페이지에서 데이터 로딩 시 콘텐츠 형태의 플레이스홀더 표시 (BookCard / StatCard / FriendCard 타입별 스켈레톤)
- **토스트 알림**: CRUD 작업 완료 시 우상단 성공/실패/정보 알림 (3.5초 자동 소멸, glass 스타일)
- **빈 상태**: 데이터가 없을 때 float 애니메이션 아이콘 + 행동 유도 버튼
- **에러 처리**: React Query 에러 상태 + 폼 검증 에러 메시지

#### 커스텀 CSS

- **커스텀 키프레임**: `fade-in`, `slide-up`, `scale-in`, `shimmer`, `slide-in-right`, `bounce-in`, `float`, `pulse-ring` (8종)
- **스프링 커브**: `cubic-bezier(0.16, 1, 0.3, 1)` / `cubic-bezier(0.34, 1.56, 0.64, 1)` 적용
- **`.stagger-children`**: CSS만으로 자식 요소 순차 등장 (50ms 간격, 최대 10개)
- **`.glass` / `.glass-strong`**: 반투명 + blur + saturate 유틸리티
- **커스텀 스크롤바**: 6px 얇은 보라색 스크롤바
- **고정 배경 그라디언트**: 4단계 Indigo 그라디언트 (`background-attachment: fixed`)
...
| 중간 | 페이지 전환 애니메이션 | React Router + Framer Motion 라우트 트랜지션 |
...
| 버전 | 날짜 | 내용 |
|------|------|------|
| v1.0 | 2026.02 | MVP 완성 — 도서/친구/대여/독후감 CRUD, Firebase 인증 |
| v1.1 | 2026.02 | Premium UI/UX — Indigo-Violet 팔레트, glass-morphism, 그라디언트, 애니메이션 |
| v1.2 | 2026.02 | 통계 대시보드 — 카테고리 분포, 월별 대여, 상위 대여자, SVG 도넛 차트 |
| v1.3 | 2026.02 | UI/UX 고도화 — 스켈레톤 로딩, 토스트 알림, bounce-in/float 애니메이션, 마이크로 인터랙션 |
